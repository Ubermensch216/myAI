  ...
  sections: [{
    ...
    children: [
      ...
      featureTable([
        ...
        ['내부검토', '부서 노트북·첨부 문서 기반 컴플라이언스 검토 및 보고서 생성'],
        ['스튜디오', '첨부 문서 마인드맵, 노트북 지식그래프, 구조화 문서 편집기, 파일 도구'],
        ['일정', '한국 공휴일 기반 로컬 캘린더 에이전트'],
        ['설정', '개인 설정(이름·테마·아바타) 및 어드민 콘솔(노트북 관리·RAG 평가·통계)']
      ]),

      h2('1.3 화면 구성'),
      para('리걸데스크 화면은 3개 영역으로 구성됩니다.'),
      featureTable([
        ['왼쪽 사이드바', '대화방 목록, 법령검토·내부검토·일정 메뉴, 설정 버튼'],
        ['중앙 콘텐츠', '채팅 메시지, 검토 결과, 일정 등 주요 콘텐츠 표시 영역'],
      ...
      h2('2.3 새 대화 화면 및 부서 노트북 지식그래프'),
      para('새 대화 시작 시 Cat_Coder 마스코트가 표시됩니다. 오른쪽 스튜디오에서 부서 노트북의 지식그래프를 조회하여 주요 개념, 법령, 절차 간의 관계를 탐색할 수 있습니다.'),
      imgPara('03.대화(프로젝트 지식그래프).png', 1577, 901, 560),
      caption('[그림 2-3] 새 대화 화면 및 지식그래프 (공무원 복무규정 노트북)'),
      ...
      bullet('법령 명칭 및 시행일'),
      bullet('인용 유형 구분: 공식 법령 [L], 헌재 결정 [D], 행정심판 결정 [D], 노트북 [N]'),
      ...
      h2('5.1 내부검토 프로세스'),
      bullet('1단계: 기준 문서(내부 규정, 표준 약관 등) 업로드 또는 부서 노트북 선택'),
      ...
      featureTable([
        ...
        ['부서 노트북 연동', '부서 RAG 노트북의 법령·지침을 검토 근거로 활용'],
      ...
      h2('6.3 지식그래프'),
      para('부서 노트북의 Knowledge Graph를 시각화합니다. 법령, 절차, 개념 간의 관계를 노드·엣지 형태로 탐색할 수 있습니다. 노드를 클릭하면 관련 출처 및 설명이 표시됩니다.'),
      featureTable([
      ...
      bullet('부서 노트북은 서버에 저장되며, 어드민 승인을 통해 AI 생성 문서를 노트북에 추가할 수 있습니다.'),
