...
- Korean Law Engine covers every roadmap phase: statute search, article retrieval, citation verification, official-source research (precedents/legal interpretations/admin rules/ordinances), impact maps, time-travel comparison (`/article/at`, `/article/diff`, `/history` + Studio Law Explorer "조문 이력" UI), `action_plan` mode with mandatory non-legal-advice disclaimer, and notebook knowledge-graph integration (deterministic `Statute`/`Article` harvest + answer-time `LawApiClient` enrichment via `mergeLawContexts`).
