...
import {
  ...
  buildDecisionSearchQueries
} from "./lawDecisionQuery.js";

// 검색어 유틸리티는 정보탐색(lawContextBuilder)과 공유한다. 외부/테스트 호환을 위해 재export.
export { extractCaseNumbers, isCaseNumberQuery, extractContentKeywords, buildDecisionSearchQueries };
...
export async function buildLawWorkbench(input = {}, options = {}) {
  ...
  const [
    ...
    runDecisionSearch("precedents", (q) => client.searchPrecedents({ query: q, display: 5, search: isCaseNumberQuery(q) ? 1 : 2 }, { signal }), decisionQueries, { warnings, errors }),
