...
export async function createNotebook({ name, description }) {
  ...
  if (!cleanName) throw new Error("지식팩 이름이 필요합니다.");
  if (cleanName.length > 80) throw new Error("지식팩 이름은 80자 이내여야 합니다.");
...
export async function addNotebookDocument(notebookId, parsedDocument, onProgress = null) {
  const manifest = await readManifest(notebookId);
  if (!manifest) throw new Error("지식팩을 찾을 수 없습니다.");
  if (!parsedDocument || parsedDocument.kind !== "document") {
    throw new Error("이미지가 아닌 문서 파일만 지식팩에 추가할 수 있습니다.");
  }
  ...
  if (
    ...
    throw new Error(
      `임베딩 차원 불일치: 지식팩=${manifest.embedding.dim}, 새 문서=${ingestResult.dim}. 동일한 임베딩 모델을 사용하세요.`
    );
  }
