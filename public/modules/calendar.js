...
export async function submitCalendarCommand(formEvent) {
  formEvent.preventDefault();
  ...
  try {
    ...
    showCalendarCommandResult({ text: outcome.text, kind: outcome.kind || "info", events: outcome.eventCards || [] });
    elements.calendarCommandInput.value = "";
  } finally {
...
export function setCalendarCommandBusy(busy) {
...
export function showCalendarCommandResult({ text, kind = "info", events = [] }) {
  ...
  body.append(textEl);
  if (Array.isArray(events) && events.length) body.append(renderEventCardList(events));
  elements.calendarCommandResult.hidden = false;
}

export function hideCalendarCommandResult() {
  if (!elements.calendarCommandResult) return;
